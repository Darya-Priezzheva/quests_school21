...
t_fdnode *find_or_create_fdnode(int fd, t_fdnode **fdlist)
{
    t_fdnode *current_node = *fdlist;
    ...
    t_fdnode *new_node = malloc(sizeof(t_fdnode));
    new_node->fd = fd;
    new_node->buf = NULL;
    new_node->is_binary = 0;
    new_node->next = *fdlist;
    *fdlist = new_node;

    return new_node;
}

void remove_fdnode(int fd, t_fdnode **fdlist)
{
    t_fdnode *current = *fdlist;
    t_fdnode *prev = NULL;
    while (current != NULL) {
        if (current->fd == fd) {
            if (prev == NULL) {
